...
import az.btb.mobilebanking.models.OrderPlasticCardProduct;
import az.btb.mobilebanking.models.ProductOrdererInfo;
import az.btb.mobilebanking.models.ProductPlasticCardOrderRequest;
...
@InjectViewState
public class ProductOrderPlasticCardPresenter extends MvpPresenter<ProductOrderPlasticCardView> {

    private final Router router;
    private final AuthService authService;
    private final CompositeDisposable compositeDisposable = new CompositeDisposable();

    @Inject ProductOrderPlasticCardPresenter(final Router router, final AuthService authService) {
        this.router = router;
    ...
    void makePlasticCardOrder(int productId, ProductOrdererInfo payerInfo, int orderType, int currency, int cardTerm) {
        ProductPlasticCardOrderRequest request = new ProductPlasticCardOrderRequest(
            Utils.getCommonRequest(),
            new OrderPlasticCardProduct(
                payerInfo,
                orderType,
                productId,
                currency,
        ...
        compositeDisposable.add(
            authService
                .orderPlasticCard(request)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(
                    response -> {
                        if (response.getResponseInfo().getResponseType() == 0)
                            getViewState().showOrderResult(response.getPlasticCardOrderStatus());
                        else
                            getViewState().showError(response.getResponseInfo().getResponseMessage());
                    },
                    error -> getViewState().showError(error.getMessage())
